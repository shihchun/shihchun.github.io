...
在查找表裡面會記錄inode nuber（ID號碼）、修改時間、權限以及要抓資料的區塊位置。Linux中檔案在開啟的時候就是利用這個概念去抓資料的，首先讀取到inode number，再確認權限，如果不匹配就直接拒絕，匹配的話就會把數據塊中的資料抓給使用者使用。

![圖3-1.1 Linux檔案系統抓資料示意圖](/uploads/linux/inode.jpg)
