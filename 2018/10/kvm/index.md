...
....Libvirt可以管理KVM，透過Libvirt可以使用Openstack做雲端計算，實現位置監控以及自動化

<!-- more -->

![KVM structure](/media/Kernel-based_Virtual_Machine.svg)
...
通常如果User Space使用率高是正常的，如果Kernel Space（sys）的部分使用率過高的話就要注意一下了。
...
```
![kvm 安裝](/media/kvm_install2.JPG)

在經過折騰一陣之後我發現直接用`virt-manager`安裝就行了，指令安裝可能有它的好處在吧！
...
![kvm manager](/media/kvm_install.JPG)
