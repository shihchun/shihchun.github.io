...
```bash
[user@localhost ~]$ locate install.log
/user/install.log
/user/install.log.syslog
[user@localhost ~]$ touch G-3Gumdam.plist
[user@localhost ~]$ locate G-3Gumdam.plist
## 搜不到任何檔案
[user@localhost ~]$ updatedb ## 強制更新locate的資料庫
[user@localhost ~]$ locate G-3Gundam.plist
...
```bash
[user@localhost ~]$ vi /etc/locate.rc
...
```bash
[user@localhost ~]$ locate swtag.log ## 搜索/tmp下的檔案
##搜尋不到任何文件
```
...
```bash
[user@localhost ~]$ which npm #搜索一個外裝指令
# 搜不到任何東西
```
...
```bash
[user@localhost ~]$ export PATH=/usr/local/webserver/mysql/bin:$PATH
#新增環境變量
[user@localhost ~]$ echo $PATH #查看設定的環境變量
...
```bash
[user@localhost ~]$ find / -name install.log
/root/install.log
```
...
```bash
[user@localhost ~]$ find /root -name ly*
##搜索範圍內ly開頭的文件
/root/lyre
/root/lyri
/root/lyris
[user@localhost ~]$ find /root -name lyr?
##搜索範圍內ly后一個字符的文件
/root/lyri
[user@localhost ~]$ find /root -name lyr[ie]$
##搜索範圍內lyr后為i或為e的文件
/root/lyre
/root/lyri
[user@localhost ~]$ find /root -name *[ie]$
##搜範圍內索結尾為i或是為e的文件
/root/lyre
/root/lyri
[user@localhost ~]$find ~/Desktop/Tools -name "*"
##搜索範圍內所有文件
...
[user@localhost ~]$ find /root -name lyris*
```
...
[user@localhost ~]$ find / -iname InStAlL.log
/root/install.log
```
...
```bash
[user@localhost ~]$ find /root -user root
/root
/root/.bashrc
/root/.tcshrc
/root/.bash_history
...
```bash
[user@localhost ~]$ find /root -nouser root
/root/Gumdam.out
[user@localhost ~]$ find /root -nouser -exec rm -rf {} \;
#搜索並且刪除所有沒有所有者的文件
```
...
```bash
[user@localhost ~]$ find /var/log -mtime +10
```

其中`10`代表天數，如果加上正負號的話，`+10`代表我要查找十天之前修改的文件、`10`代表十天當天修改的文件、`-10`代表十天內修改的文件。


![mtime](/media/mtime.png)


## find / -atime 搜索文件上次訪問的時間

```bash
[user@localhost ~]$ find /var/log -atime +10
```

## find / -ctime 搜索文件上次修改文件屬性的時間

```bash
[user@localhost ~]$ find /var/log -ctime +10
```

## find /  -size 以文件大小搜索文件

```bash
[user@localhost ~]$ find . -size 25k
```
...
![inode](/media/inode.jpg)


## find / -inum 以inode numbr搜索文件

```bash
[user@localhost ~]$ find / -inum 262223
```
...
```bash
[user@localhost ~]$ find /etc -size +32k -a -mtime -10
## 搜索/etc 目錄下大於32KB且十天內有修改過內容的文件
[user@localhost ~]$ find /etc -size +32k -o -mtime -10
...
```bash
[user@localhost ~]$ find /etc -size +32k -exec ls -lh {} \;
## 列出/etc目錄下面大於32KB的檔案
[user@localhost ~]$ find /etc -nouser -exec rm -rf {} \;
...
```bash
[user@localhost ~]$ ll ~/Desktop/te.md -exec mv ~ {} \;
...
```bash
[user@localhost ~]$ grep "net" anaconda-ks.cfg
network  --bootproto=dhcp --device=ens33 --ipv6=auto --activate
network  --hostname=localhost.localdomain
[user@localhost ~]$ grep -i "net" anaconda-ks.cfg
# Network information
network  --bootproto=dhcp --device=ens33 --ipv6=auto --activate
network  --hostname=localhost.localdomain
```
...
```bash
[user@localhost ~]$ echo maplestory is great game >> info.log
[user@localhost ~]$ grep -v maplestory info.log
[user@localhost ~]$ echo you lose &>> info.log
[user@localhost ~]$ cat info.log
