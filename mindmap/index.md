...
[unordered-list-to-mind-map](https://github.com/HunterXuan/unordered-list-to-mind-map)放到`/static/mind/`目錄下面，然後加入`/layouts/shortcodes/mind.html`的全局shortcode，關於這個shortcode我要說一下，在我這裡`{{ .Inner }}`不行用，要用`{{ .Inner | markdownify }}`，就看一下結果就知道了，就`mindmap.js`檔案寫的內容，如果可以改用`#`更好，不過`<h1><h2><h3>`這種tag看起來寫起來就很麻煩。
